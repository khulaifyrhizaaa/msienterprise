...
class Dataguru extends BaseController
{
    ...
    public function index()
    {
        ...
        $data['dosen0072'] = $this->model->where($where)->like($like)->orLike($or_like)->paginate($paginate, 'dataguru');
        $data['pager'] = $this->model->pager;
        // generate number untuk tetap bertambah meskipun pindah halaman paginate 
        $nomor = $this->request->getGet('page_dataguru');
        ...
        echo view('dataguru/idx3', $data);
    }

    public function create3()
    {
        # code...
        return view('dataguru/create3');
    }
    ...
    public function store3()
    {
        ...
        if ($validation->run($data, 'dataguru') == FALSE) {
            session()->setFlashdata('inputs', $this->request->getPost());
            session()->setFlashdata('errors', $validation->getErrors());
            return redirect()->to(base_url('dataguru/create3'));
        } else {
    ...
    public function update3()
    {
        ...
        if ($validation->run($data, 'dataguru') == FALSE) {
            session()->setFlashdata('inputs', $this->request->getPost());
            session()->setFlashdata('errors', $validation->getErrors());
            return redirect()->to(base_url('dataguru/edit3/' . $id));
        } else {
            $model = new Dataguru_model();
            $filename->move(ROOTPATH . 'public/uploads', $name);
